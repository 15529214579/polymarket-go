...
WHALE_FINGERPRINT_SOURCES="${WHALE_WALLETS_FILE:-$ROOT/wallets.strategy-push.txt} ${WHALE_EXTRA_WALLETS_FILES:-$ROOT/wallets.football-score-push.txt $ROOT/wallets.leaderboard-push.txt $ROOT/wallets.leaderboard-watch.txt $ROOT/wallets.leaderboard-sports-push.txt $ROOT/wallets.sports-holders-push.txt $ROOT/wallets.hourly-push.txt}"
WHALE_FINGERPRINT_EXCLUDES="${WHALE_EXCLUDE_WALLETS_FILES:-$ROOT/wallets.strategy-quarantine.txt $ROOT/wallets.strategy-review-noise.txt $ROOT/db/strategy_iteration/wallets.strategy-exclude.txt}"
PAPER_FINGERPRINT_SOURCES="${SMARTMONEY_PAPER_SOURCE_WALLETS:-$ROOT/db/smartmoney-paper/wallets.paper-promoted.txt $ROOT/wallets.football-score-push.txt $ROOT/wallets.strategy-push.txt $ROOT/wallets.hourly-push.txt $ROOT/wallets.leaderboard-watch.txt $ROOT/wallets.leaderboard-sports-push.txt}"
PAPER_FINGERPRINT_EXCLUDES="${SMARTMONEY_PAPER_EXCLUDE_WALLETS:-$ROOT/db/smartmoney-paper/wallets.paper-demoted.txt $ROOT/wallets.strategy-quarantine.txt $ROOT/wallets.strategy-review-noise.txt $ROOT/db/strategy_iteration/wallets.strategy-exclude.txt}"
...
wallet_fingerprint() {
  ...
  python3 "$ROOT/scripts/wallet_set_fingerprint.py" "${args[@]}"
}

whale_fingerprint_before="$(wallet_fingerprint address-list "$WHALE_FINGERPRINT_SOURCES" "$WHALE_FINGERPRINT_EXCLUDES")"
paper_fingerprint_before="$(wallet_fingerprint address "$PAPER_FINGERPRINT_SOURCES" "$PAPER_FINGERPRINT_EXCLUDES")"
...
lines = [
    "# generated by hourly-wallet-promotion",
...
whale_fingerprint_after="$(wallet_fingerprint address-list "$WHALE_FINGERPRINT_SOURCES" "$WHALE_FINGERPRINT_EXCLUDES")"
whale_effective_changed=0
if [ "$whale_fingerprint_before" != "$whale_fingerprint_after" ]; then
  whale_effective_changed=1
fi

if [ "${HOURLY_PROMOTION_RESTART:-1}" = "1" ] && [ "$whale_effective_changed" = "1" ]; then
  ...
  printf 'hourly-wallet-promotion.restart_skipped worker=whale output_changed=%s effective_changed=%s\n' "$changed" "$whale_effective_changed"
fi
...
if [ "${HOURLY_SMARTMONEY_REFRESH:-1}" = "1" ] && [ -x "$ROOT/scripts/start-smartmoney-paper.sh" ]; then
  ...
  paper_fingerprint_after="$(wallet_fingerprint address "$PAPER_FINGERPRINT_SOURCES" "$PAPER_FINGERPRINT_EXCLUDES")"
  paper_effective_changed=0
  if [ "$paper_fingerprint_before" != "$paper_fingerprint_after" ]; then
    paper_effective_changed=1
  fi
  if [ "$paper_effective_changed" = "0" ]; then
    printf 'hourly-wallet-promotion.restart_skipped worker=smartmoney effective_changed=0\n'
  elif "$ROOT/scripts/start-smartmoney-paper.sh" restart; then
    printf 'hourly-wallet-promotion.smartmoney refreshed effective_changed=1\n'
  else
