...
func TestPaperTimeoutExitPriceRequiresBestBid(t *testing.T) {
	tests := []struct {
		...
		{name: "best bid", tick: feed.Tick{BestBid: 0.28, Mid: 0.40}, want: 0.28, ok: true},
		{name: "no mid fallback", tick: feed.Tick{Mid: 0.40}, ok: false},
		{name: "invalid bid", tick: feed.Tick{BestBid: 1, Mid: 0.40}, ok: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, ok := paperTimeoutExitPrice(tt.tick)
