...
func main() {
	...
	flag.IntVar(&cfg.HTTPMaxAttempts, "http_attempts", def.HTTPMaxAttempts, "maximum HTTP attempts for transient failures")
	flag.DurationVar(&cfg.HTTPRetryBase, "http_retry_base", def.HTTPRetryBase, "initial HTTP retry delay")
	flag.DurationVar(&cfg.HTTPRetryMax, "http_retry_max", def.HTTPRetryMax, "maximum HTTP retry delay")
	flag.DurationVar(&cfg.HTTPTimeout, "http_timeout", def.HTTPTimeout, "timeout per HTTP request")
