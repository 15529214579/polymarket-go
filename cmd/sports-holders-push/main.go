...
func writeWallets(path string, rows []*holderAgg, markets int, minShares float64) error {
	...
	for _, row := range rows {
		fmt.Fprintf(&b, "%s # list=sports_holders_push tier=%s smart=%.1f bot=%.1f shares=%.0f maxShares=%.0f markets=%d categories=%s risks=%s top=%s\n",
			row.Wallet, dash(row.Tier), row.Smart, row.Bot, row.Shares, row.MaxShares, row.Markets, categorySummary(row.Categories), riskSummary(row.Risks), cleanComment(row.TopMarket))
	}
