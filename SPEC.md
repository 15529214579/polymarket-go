...
- `com.polymarket-go.daily-p0-optimize` 每天本地时间 04:30 在日报稳定后运行。
- 内层 Codex 在隔离的临时 Git worktree 中最多处理一个有运行证据的正确性、风险控制、数据完整性、可靠性或可观测性 P0，不直接接触正在更新的主工作区，也不负责 Git 状态或网络部署。
...
- 实盘开关、运行数据库、生成报表、地址池以及策略/金额/退出/晋升阈值继续由人工控制。
