...
- 自动交易进程不得持有链上交易发送能力；Polygon 余额查询使用只读客户端，wrap/approve/redeem 必须由独立维护动作执行后退出
- 实盘链上维护与模拟盘隔离：赎回状态、锁和授权只允许放在 gitignored 的 `db/live/`；不得读取或覆盖模拟盘仓位、journal、PnL 和风控文件
...
## P12. 每日数据迭代（2026-04-28）

每 24 小时自动总结交易数据、分析算法表现、生成迭代建议。

**触发时间**：SGT 00:05（cron → `scripts/daily-iterate.sh`）

**分析维度**：
1. **总览**：7 天滚动窗口，胜率/PnL/持仓时间/SL-TP-Timeout 触发率
2. **每日明细**：逐日笔数/胜率/PnL 趋势
3. **品类分析**：LoL/NBA/Dota2/Tennis/Football/Other 分别统计
4. **出场分析**：ladder_sl/tp1/tp2/timeout 各自的次数和 PnL 贡献
5. **入场价带**：0.05-0.15/0.15-0.25/...各区间的胜率和 PnL
6. **算法建议**：自动生成（胜率偏低→收紧入场、SL 过紧→放宽、品类亏损→暂停）

**输出**：
- `reports/daily/YYYY-MM-DD.md` — 完整 Markdown 报告（git 追踪）
- Telegram 推送 — 精简摘要（通过 PUSH_BOT_TOKEN）

**迭代闭环**：报告写到 md → 老板看到建议 → 拍板调参 → 下一天自动验证效果。

**Why**：不等老板来问"跑得怎么样"，每天主动出数据+建议。参数调整有数据支撑不凭感觉。
