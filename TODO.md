...
- [x] 每日 04:30 运行一轮 7 天保守迭代：Day 1 建基线，Day 2/4/6 最多各接受一个小改动，Day 3/5 只观察，Day 7 只复盘；全周最多 3 次，两次改动至少间隔 48h。
- [x] Codex 不能提交或推送，由外层控制器执行范围检查、敏感信息扫描、`go test -race ./...`、失败隔离、commit 和 push；没有达到数据门槛时明确 no-action。
- [x] 自动优化禁止修改实盘开关、运行数据库、地址池、金额/晋升/总敞口及自身控制文件；模拟盘/影子参数仅在至少 30 个成熟独立样本、两个非重叠切片扣完双边手续费和滑点后都改善时，允许单参数最多相对调整 10%。
...
- [x] Phase 3.0 前置：USDC.e → pUSD wrap（老板自行操作）
- [x] 2026-04-21 00:2x — **Phase 3.c 钱包 + EIP-712 V2 签名骨架**
- [x] 2026-04-28 19:1x — CLOB V2 REST `/order` POST 客户端 + V2ExchangeAddress + NegRiskExchangeAddress 填入 + L1/L2 Auth（commit `d7d7728`）
- [x] 2026-04-28 19:3x — 成交回执轮询 + status 机：GET /order/{id} 2s 间隔轮询，30s 超时自动取消；CancelOrder DELETE /order/{id}；4 处 Submit 调用点加 StatusFilled 检查
- [x] 2026-04-28 19:1x — **Apr 28 cutover WSS 烟测通过**（V2 切换后 WSS 帧结构完全兼容）
- [x] 2026-04-28 19:3x — daemon 脚本 `-live` 开关：`POLYMARKET_LIVE=1` 环境变量启用实盘
...
- [x] 2026-04-20 18:04 — Paper 友好化两件：① 非 moneyline 市场过滤加严（exact score / halftime / end in draw / handicap 全排除，LoL 的 Game Handicap + Games Total 不再订阅）；② `risk.BlockFeedSilence` 在 feed 恢复健康（<15s silent）时 auto-resume，不再等手动 Resume——paper 模式下 WSS 抖动不再熄火，日亏损 + 手动暂停仍是人判。daemon 用新 bin 重启 pid=3529。
